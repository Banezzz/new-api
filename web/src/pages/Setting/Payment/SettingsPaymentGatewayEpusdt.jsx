  ...
  useEffect(() => {
    if (props.options && formApiRef.current) {
      const currentInputs = {
        ...
        EpusdtSecretKey: '',
        EpusdtCurrency: props.options.EpusdtCurrency || 'usd',
        EpusdtToken: props.options.EpusdtToken || 'usdt',
        EpusdtNetwork: props.options.EpusdtNetwork || 'tron',
        EpusdtPaymentType: props.options.EpusdtPaymentType || 'GMPAY',
  ...
  const submitEpusdtSetting = async () => {
    ...
    try {
      const options = [
        ...
        {
          key: 'EpusdtCurrency',
          value: inputs.EpusdtCurrency || 'usd',
        },
        {
          key: 'EpusdtToken',
          value: inputs.EpusdtToken || 'usdt',
        },
        {
          key: 'EpusdtNetwork',
          value: inputs.EpusdtNetwork || 'tron',
        },
        {
          key: 'EpusdtPaymentType',
          value: inputs.EpusdtPaymentType || 'GMPAY',
        },
        {
          key: 'EpusdtNotifyURL',
          value: notifyURL,
        ...
        <Form.Section text={sectionTitle}>
          ...
          >
            ...
            <Col xs={24} sm={24} md={8}>
              <Form.Input
                field='EpusdtPaymentType'
                label={t('支付类型')}
                placeholder='GMPAY'
                extraText={t('保持 GMPAY 可接收 EPUSDT JSON 回调')}
              />
            </Col>
          </Row>
          <Row
