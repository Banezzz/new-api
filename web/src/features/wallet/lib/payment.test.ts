...
describe('payment type classification', () => {
  test('keeps Waffo and Waffo Pancake on their dedicated flows', () => {
    ...
    assert.equal(isInfiniPayment(PAYMENT_TYPES.INFINI), true)
    assert.equal(isInfiniPayment(`${PAYMENT_TYPES.INFINI}_card`), true)
    assert.equal(isEzpayPayment(PAYMENT_TYPES.EZPAY), true)
  })
})
