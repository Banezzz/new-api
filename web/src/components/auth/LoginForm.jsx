...
const LoginForm = () => {
  ...
  const renderOAuthCircles = () => {
    ...
    if (status.passkey_login && passkeySupported) {
      oauthButtons.push(
        <button key='passkey' onClick={handlePasskeyLogin} className='w-10 h-10 rounded-full border border-semi-color-border flex items-center justify-center hover:bg-semi-color-fill-1 transition-colors' title={t('使用 Passkey 登录')}>
          <IconKey />
        </button>
      );
    }
    ...
    <div className='min-h-screen flex'>
      ...
      <div className='w-full lg:w-1/2 flex items-center justify-center bg-semi-color-bg-0 p-6 md:p-12'>
        <div className='w-full max-w-sm'>
          ...
          <h1 className='text-2xl font-bold text-semi-color-text-0 mb-2'>{t('登录')}</h1>
          <p className='text-sm text-semi-color-text-2 mb-8'>{t('登录以继续使用')}</p>
          ...
          {/* Email/username form - always shown */}
          ...
          {!status.self_use_mode_enabled && (
            <p className='text-center text-sm text-semi-color-text-2 mt-6'>
              {t('没有账号？')}{' '}
              <Link to='/register' className='text-semi-color-primary font-medium hover:opacity-80'>
                {t('注册')}
              </Link>
            </p>
          )}
