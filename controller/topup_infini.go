...
func handleInfiniOrderCompleted(c *gin.Context, event *service.InfiniWebhookOrderEvent, payload string) {
	...
	if err := model.CompleteSubscriptionOrder(tradeNo, payload, model.PaymentProviderInfini, ""); err == nil {
...
func handleInfiniOrderExpired(c *gin.Context, event *service.InfiniWebhookOrderEvent) {
	...
	err = model.ExpireSubscriptionOrder(tradeNo, model.PaymentProviderInfini)
	if err == nil {
	...
	err = model.UpdatePendingTopUpStatus(tradeNo, model.PaymentProviderInfini, common.TopUpStatusExpired)
